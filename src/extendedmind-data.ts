export class ExtendedMindPublicItems {
  ...
  addKeywordsToExternalNote(note){
    this._addTagsToNote(note);
    if (note.keywords && note.keywords.length){
      let parentKeywords = [];
      for (let i=0; i<note.keywords.length; i++){
        if (note.keywords[i].parent){
          let parentTag = this._getTagByUUID(note.keywords[i].parent);
  ...
  _updateNote(note){
    ...
    for (let i=0; i<this.notes.length; i++){
      if (this.notes[i].uuid === note.uuid){
        ...
        this.notes[i].description = note.description;
        this.notes[i].link = note.link;
        this.notes[i].content = note.content;
        this.notes[i].visibility = note.visibility;
